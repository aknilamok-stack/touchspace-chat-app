...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (session.role !== "manager_supervisor") {
  ...
  const canCurrentManagerWriteToChat = useCallback(
    (chat: ChatItem | null) => {
      if (!chat || chat.rawStatus === "resolved" || !managerSupervisorPowerEnabled) {
        return false;
      }
  ...
  useEffect(() => {
    ...
    const activeCandidateIds = new Set(
      notificationCandidates.map((candidate) => candidate.notificationKey)
    );
  ...
  useEffect(() => {
    ...
    if (!managerSupervisorPowerEnabled || actionableNotifications.length === 0) {
      document.title = defaultDocumentTitleRef.current;
      return;
    }
  ...
  }, [activeChatId, notificationCandidates, managerSupervisorPowerEnabled]);
            ...
            <div className="mx-auto w-full max-w-3xl">
              {activeChat?.rawStatus === "resolved" ? (
                ...
                <div className="space-y-3">
                  ...
                  {!managerSupervisorPowerEnabled ? (
                    <p className="text-sm text-[#8E8E93]">
                      Молния выключена: управленец может читать диалоги, но не может подключаться к ним и писать.
                    </p>
                  ) : null}
                </div>
	              ) : (
